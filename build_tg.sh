...
$SUDO dnf -y install git gcc gcc-c++ automake autoconf libtool libwayland-server-devel libxcb-devel yasm libogg-devel lzma-devel xz-devel libappindicator-devel libunity-devel libstdc++-devel libstdc++-static libwebp-devel libpng-devel xorg-x11-util-macros gettext-devel bison doxygen cmake libICE-devel libSM-devel libXi-devel
...
cd "$QTDIR"
OPENSSL_LIBS='-L/usr/local/ssl/lib -lssl -lcrypto' ./configure -prefix "/usr/local/Qt-${_QTVERSION}" -release -opensource -confirm-license -qt-zlib -qt-libpng -qt-libjpeg -system-freetype -qt-harfbuzz -qt-pcre -qt-xcb -qt-xkbcommon-x11 -no-opengl -no-gtkstyle -static -openssl-linked -I /usr/local/ssl/include -L /usr/local/ssl/lib -nomake examples -nomake tests
make $ARGS
$SUDO make install
